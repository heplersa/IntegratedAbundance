...
load("WAprevalence/output/mcmc/MCMC_no_covariates_no_pmp_2026_02_16.Rda")

# IMPORT SHAPE FILES FOR WA COUNTIES
load("WAprevalence/data/shape_county_WA.Rda")

# EXAMINE MCMC CONVERGENCE
MCMCvis::MCMCtrace(samples, params = paste0("pi[", sample(1:234, 20), ", 1]"), ISB = F, filename = "death", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("pi[", sample(79:234, 20), ", 2]"), ISB = F, filename = "ed", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("pi[", sample(1:234, 20), ", 3]"), ISB = F, filename = "hosp", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("N[", sample(1:234, 20), "]"), ISB = F, filename = "N", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("lambda[", sample(1:234, 20), "]"), ISB = F, filename = "lambda", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("f[", sample(1:234, 20), ", 1]"), ISB = F, filename = "f_death", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("f[", sample(79:234, 20), ", 2]"), ISB = F, filename = "f_ed", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("f[", sample(1:234, 20), ", 3]"), ISB = F, filename = "f_hosp", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("v[", sample(1:234, 20), "]"), ISB = F, filename = "v", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("u[", sample(1:234, 20), "]"), ISB = F, filename = "u", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("beta[", 1:7, ", 1]"), ISB = F, filename = "beta_death", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("beta[", 3:7, ", 2]"), ISB = F, filename = "beta_ed", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("beta[", 1:7, ", 3]"), ISB = F, filename = "beta_hosp", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = "beta.mu", filename = "beta.mu", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = "mu", filename = "mu", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = "tau.f", filename = "tau.f", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = "tau.u", filename = "tau.u", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("eps[", sample(1:234, 20), ", 1]"), ISB = F, filename = "eps_death", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("eps[", sample(1:234, 20), ", 2]"), ISB = F, filename = "eps_ed", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = paste0("eps[", sample(1:234, 20), ", 3]"), ISB = F, filename = "eps_hosp", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
MCMCvis::MCMCtrace(samples, params = "cov.eps", filename = "cov.eps", wd = "WAprevalence/output/robustness_checks/no_pmp/diagnostics")
...
death_lwr <- which(names(results[[1]])=="pi[1, 1]")
death_upr <- which(names(results[[1]])=="pi[273, 1]")
ed_lwr <- which(names(results[[1]])=="pi[1, 2]")
ed_upr <- which(names(results[[1]])=="pi[273, 2]")
hosp_lwr <- which(names(results[[1]])=="pi[1, 3]")
hosp_upr <- which(names(results[[1]])=="pi[273, 3]")
...
beta_upr <- which(names(results[[1]]) == "beta[7, 3]")
...
write.csv(death_results_csv,
          file = "WAprevalence/output/robustness_checks/no_pmp/tables/death_results.csv",
          row.names = F)

write.csv(ed_results_csv,
          file = "WAprevalence/output/robustness_checks/no_pmp/tables/ed_results.csv",
          row.names = F)

write.csv(hosp_results_csv,
          file = "WAprevalence/output/robustness_checks/no_pmp/tables/hosp_results.csv",
          row.names = F)

write.csv(N_results_csv,
          file = "WAprevalence/output/robustness_checks/no_pmp/tables/N_results.csv",
          row.names = F)

write.csv(N_prev_results_csv,
          file = "WAprevalence/output/robustness_checks/no_pmp/tables/N_prev_results.csv",
          row.names = F)
...
unsampled_params <- c(which(colnames(samples)=="pi[1, 2]"):which(colnames(samples)=="pi[78, 2]"),
                      which(colnames(samples)=="f[1, 2]"):which(colnames(samples)=="f[78, 2]"),
                      which(colnames(samples)=="beta[1, 2]"):which(colnames(samples)=="beta[2, 2]"))
...
ggsave(filename = "death.png", 
       plot = death_map, 
       path = "WAprevalence/output/robustness_checks/no_pmp/maps", 
       bg = "White",
       dpi = "retina",
       height = 3,
       width = 10)
...
post_pois_outcome_prev <- apply(samples[, (beta_lwr+9):(beta_upr)], 2, exp)
...
outcomes <- c("Death due to opioid misuse",
...
ggsave(filename = "beta_log.png",
       path = "WAprevalence/output/robustness_checks/no_pmp",
       dpi = "retina",
       width = 13,
       height = 10,
       units = "cm"
