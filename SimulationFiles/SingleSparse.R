...
###################################################################################################################################
##################### Implement MCMC algorithm for the single outcome model with sparse survey data cases #########################
...
for(MM in 1:4){
  filename = paste("SingleSparse.", toString(MM), ".Rda", sep="")
  outfile = paste("SingleSparseLog.",toString(MM),".txt",sep="")
  ...
  SimMCMC<-foreach(kk=1:25) %dorng% {
    ...
    ### only observe survey data in years 2,5,8
    ell = c(2,5,8)
    S0 = S[ell,k]
    P=P[1:(TT*n)]
    ...
    mod_data=list(y1=Y[,1],S=S0,ell=ell,X=X,num=num,adj=adj,W=W)
    ...
    mod_inits=list(N=Ninit,beta.D=rep(-6,T),beta=c(.05,-.001),tau.v=.1,tau.f=.5,tau.u=.5,
                   u=rep(0,n*T),v=rep(0,n*T),f=rep(0,n*T),tau.D=.1,eps.D=rep(0,n*T),
                   gamma=rep(0,length(W[1,])),phi.u=.5,phi.f=.5)
    ...
    # Set up samplers.
    mcmc_conf <- configureMCMC(nimble_model,monitors=c('beta.D','beta','N','u','f','gamma','lambda'),control=list(adaptive=TRUE,adaptInterval=100,sliceWidths=rep(100,T),sliceMaxSteps=10000,maxContractions=10000000,sliceAdaptWidthMaxIter=0,sliceAdaptFactorMaxIter=0),useConjugacy = TRUE)
    
    ### use AF Slicei sampler for N_{i1},...,N_{iT}
    for(i in 1:n){
      Nvec = rep(0,T)
      for(j in 1:T){
