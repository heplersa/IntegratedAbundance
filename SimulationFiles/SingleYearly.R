...
###################################################################################################################################
##################### Implement MCMC algorithm for the single outcome model with yearly survey data cases #########################
...
for(MM in 1:4){
  filename = paste("SingleYearly.", toString(MM), ".Rda", sep="")
  outfile = paste("SingleYearlyLog.",toString(MM),".txt",sep="")
  ...
  SimMCMC<-foreach(kk=1:25) %dorng% {
    ...
    mod_inits=list(N=Ntrue,beta.D=rep(-6,T),beta=c(.05,-.001),tau.v=.1,tau.f=.5,tau.u=.5,
                   u=utrue,v=rep(0,n*T),f=ftrue,tau.D=.1,eps.D=rep(0,n*T),
    ...
    #### use AF Slice Sampler for N_{i1},...,N_{iT}
    for(i in 1:n){
      Nvec = rep(0,T)
      for(j in 1:T){
